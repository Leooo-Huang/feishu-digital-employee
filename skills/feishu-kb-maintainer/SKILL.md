...
- **Claude Code**：`lark-cli event consume minutes.minute.generated_v1 --as user` 与 `lark-cli event consume vc.note.generated_v1 --as user` 与 `lark-cli event consume im.message.receive_v1 --as bot` 各自管道到 `bin/on-event.js`，其输出交当前 agent 按本 SKILL 处理；`bin/digest.js` 用 `/schedule` 周期触发。
