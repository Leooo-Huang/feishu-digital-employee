...
本 skill 是**大脑**：你（宿主 LLM）读本文件做语言抽取与路由决策；**手脚**是 `src/` 下的 Node 模块（封装 lark-cli）；**记忆**是飞书多维表格「路由幂等表」；状态全在 Base，每次处理都"读 Base→抽取→路由判定→调 lark-cli 写回→回写 Base"，因此无状态、可移植、抗重启。

## 四条铁律（不可违反）

1. **群消息不逐条入库**：原始群消息信噪比低。累积一个时间窗/一定消息数后再抽取决策/结论/待办/FAQ，丢弃闲聊。绝不把每条消息塞进文档。
2. **写前去重（content_hash 幂等）**：任何写入前先按 `source_id` 查路由幂等表，用 `route-io.decideRoute` 判定 write / update / skip。未变就 skip，绝不重复写或写错文档。
...
字段（设计 §4.1）：`source_type`(meeting/chat/comment/manual)、`source_id`、`source_meta`、`target_kind`(doc/task/okr/comment)、`target_id`、`target_locator`、`content_hash`、`status`(written/updated/skipped)、`last_synced_at`。
...
4. 对每个落点按 `route-io.decideRoute` 判定后写：
   - **会议纪要段** → 命中项目「会议纪要」小节：先 `kb-write.resolveWikiNode(节点)` 拿 docToken，再 `kb-write.fetchDoc`（取小节 block_id），按落点 `appendDoc`/`insertAfterBlock`/`replaceBlock`。写 `--content` 前**先 `lark-cli skills read lark-doc references/lark-doc-update.md` 和 `references/lark-doc-md.md`**，按其转义规则构造 Markdown/XML。
...
4. 命中该群对应文档「群聊沉淀」小节 → `kb-write` 更新；拍板决策汇入 Decision Log。
5. `recordRoute(... source_type:'chat' ...)`。

### 功能 3 · 待办维护（原生 Task，单一事实源）
1. 行动项 → `task-io.createTask({summary, description:来源回链, assigneeOpenId, due, idempotencyKey})`（负责人原文先用通讯录解析成 open_id；解析不到则留空并在文档注明）。
2. 需提醒 → `task-io.setReminder(taskId, '1h')`。后续变更 `task-io.updateTask`。
3. 文档侧「公司待办看板」「项目待办汇总」是**只读镜像**：`task-io.listMyTasks`/`searchTasks` 取任务 → 渲染成只读列表用 `kb-write` 写镜像页。勾选/改在 Task App 内做，不在文档里改。
4. `recordRoute(target_kind:'task', target_id:task_guid)`。

### 功能 4 · 战略目标维护（原生 OKR，单一事实源）
1. **不新建 Objective**。你识别与某 KR 相关的进展 → `okr-io.createProgress({targetId:KR id, targetType:'key_result', text:进展, percent?, status?, sourceTitle, sourceUrl})`。
2. 「公司战略与年度目标」页按周镜像：`okr-io.listCycles`→取 cycle_id→`okr-io.cycleDetail(cycleId)` 读 Objective/KeyResult → 你汇编成可读汇总 → `kb-write.appendDoc/replaceBlock` 写镜像页。
...
2. **你**把它汇编成周报正文（缺失不编造）→ `kb-write.appendDoc(周报页 docToken, 周报内容)`。
