...
export async function createTask(fields, { profile } = {}) {
  if (!fields?.summary || !String(fields.summary).trim()) {
    throw new Error('task +create 缺 summary（任务标题），拒绝建空任务（缺失不编造）');
  }
  const argv = ['task', '+create', '--summary', String(fields.summary), '--as', 'user', '--format', 'json'];
...
export async function updateTask(taskId, patch, { profile } = {}) {
  if (!taskId) throw new Error('task +update 缺 task-id');
