...
export async function createProgress(p, { profile } = {}) {
  if (!p?.targetId || !p?.targetType) throw new Error('okr +progress-create 缺 targetId/targetType');
  if (p.text == null || String(p.text).trim() === '') throw new Error('okr +progress-create 进展内容为空，拒绝写空进展（缺失不编造）');
