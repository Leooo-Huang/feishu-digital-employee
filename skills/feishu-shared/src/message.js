// 消息收发：群发/群内@/私信/加急。命令面经实测校准 (lark-cli 1.0.53)。
...
/**
 * 发群消息并在文本前 @ 指定成员（催办专用），返回发送结果（含 message_id 供加急用）。
 * @ 用飞书文本内嵌标记 <at user_id="ou_xxx"></at>（messages-send 文本即 {"text":...}，客户端按 id 渲染）。
 * openIds 为空时退化为普通群消息（不 @）。
 */
export async function sendGroupMention(chatId, text, openIds = [], { profile, idempotencyKey } = {}) {
  const ats = (openIds || []).filter(Boolean).map((id) => `<at user_id="${id}"></at>`).join(' ');
  const content = ats ? `${ats} ${text}` : text;
  const argv = ['im', '+messages-send', '--chat-id', chatId, '--text', content, '--format', 'json', '--as', 'bot'];
  if (idempotencyKey) argv.push('--idempotency-key', idempotencyKey);
  return larkJson(argv, { profile });
}
