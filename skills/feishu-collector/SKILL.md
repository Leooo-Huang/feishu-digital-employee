...
## 文档依赖
本 skill **不依赖** Wiki 知识库骨架——收集结果写回**用户上传的源文件**（sheet/base/doc），落点是用户文件而非知识库节点。只依赖状态库 Base（任务表/槽位表，由 `bin/setup-base.js` 建）。与之相对，`feishu-kb-maintainer` 依赖知识库骨架先存在（由 `feishu-init`/`kb-scaffold` 建）。
...
- **Claude Code**：用统一入口 `node skills/feishu-init/bin/init.js setup-events --start` 订阅 `im.message.receive_v1` 并 bridge 到 `bin/on-message.js`（同流也分发给 kb 的 on-event）。⚠️ 勿用裸管道 `event consume | node on-message.js`——consume 是无限流、on-message 读 stdin 到 EOF，会永久缓冲；必须走 bridge（逐行 `--event`）。`bin/tick.js` 用 `/schedule` 或 cron 周期触发。
