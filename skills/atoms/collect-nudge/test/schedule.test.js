...
import { selectSlotsToNudge, NUDGE_STATES } from '../src/schedule.js';
...
test('「收到原始」不催（球在机器人侧）', () => {
  const slots = [{ slot_id: 's1', 状态: '收到原始', 最近询问时间: '2026-06-16T10:00:00+08:00', 追问次数: 0 }];
  assert.equal(selectSlotsToNudge(slots, task, now).length, 0);
  assert.ok(!NUDGE_STATES.includes('收到原始'));
});
test('无效/缺失截止 → 仍按间隔催，但不 urgent（不崩、不被当永不到期跳过）', () => {
  const noDeadline = { reminderIntervalH: 24, maxAttempts: 3 }; // 无 deadline
  const slots = [{ slot_id: 's1', 状态: '已问', 最近询问时间: '2026-06-16T10:00:00+08:00', 追问次数: 0 }];
  const r = selectSlotsToNudge(slots, noDeadline, now);
  assert.equal(r.length, 1);
