...
test('decideRoute: 源变 + 目标被人工改过 → conflict（不自动覆盖）', () => {
  const existing = { record_id: 'rec1', content_hash: contentHash('旧'), target_content_hash: contentHash('旧'),
    target_id: 'docx_x', target_locator: 'blk_3' };
  const d = decideRoute(existing, '新', '被人改过的内容'); // 目标当前内容 ≠ 上次写入指纹
  assert.equal(d.action, 'conflict');
  assert.match(d.reason, /人工修改|确认/);
});

test('decideRoute: 源变 + 目标未被改（仍等于上次写入）→ update', () => {
  const existing = { record_id: 'rec1', content_hash: contentHash('旧'), target_content_hash: contentHash('旧'),
    target_id: 'docx_x', target_locator: 'blk_3' };
  const d = decideRoute(existing, '新', '旧'); // 目标当前内容 == 上次写入
  assert.equal(d.action, 'update');
});

test('decideRoute: 无 currentTargetContent（读不到目标）→ 跳过覆盖检查，仍 update（向后兼容）', () => {
  const existing = { record_id: 'rec1', content_hash: contentHash('旧'), target_content_hash: contentHash('旧') };
  assert.equal(decideRoute(existing, '新').action, 'update');
});

test('decideRoute: 旧记录无 target_content_hash → 用 content_hash 作基线做覆盖检查', () => {
  const existing = { record_id: 'rec1', content_hash: contentHash('旧') }; // 无 target_content_hash
  assert.equal(decideRoute(existing, '新', '旧').action, 'update'); // 目标==content_hash 基线
  assert.equal(decideRoute(existing, '新', '被改过').action, 'conflict'); // 目标≠基线
});
...
test('recordRoute(write): 创建记录并落 content_hash + status=written', async () => {
  ...
  assert.equal(fields.target_content_hash, contentHash('内容X')); // 覆盖保护基线已落
